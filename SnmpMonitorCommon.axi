...
define_function integer array_clear(char array[][][]) {
	...
	for (i = 1; i <= length_string(varbinds); i++) {
		varbinds[i][ARRAY_VALUE] = '';
	}
		...
		if ((cmd == 'OID') || (cmd == 'TRAP')) {
			...
			if (!explode(data.text, ',', '"', parts)) {
				...
				if (
					...
					if (AMX_DEBUG <= get_log_level()) debug("'data_event string ', cmd, ': procesing varbind'");
					...
					if (array_set(varbinds, parts[SNMP_RESPONSE_PARAM_OID], parts[SNMP_RESPONSE_PARAM_VALUE])) { // Avoid processing updates for unchanged values
						if (AMX_DEBUG <= get_log_level()) debug("'data_event string TRAP: ', 'OID ', parts[SNMP_RESPONSE_PARAM_OID], ' updated'");
