...
define_function connman_server_reinitialize() {
    if (AMX_DEBUG <= get_log_level()) debug("'connman_server_reinitialize', '()'");
...
define_function connman_server_close() {
    if (AMX_DEBUG <= get_log_level()) debug("'connman_server_disconnect', '()'");
    
    switch (connman_server_open_status) {
	case CONNMAN_SERVER_STATUS_CLOSED: {
	    if (AMX_DEBUG <= get_log_level()) debug("'connman_server_disconnect', '() ', 'listening socket already closed'");
	}
	case CONNMAN_SERVER_STATUS_OPENING: {
	    if (AMX_DEBUG <= get_log_level()) debug("'connman_server_disconnect', '() ', 'cancelled attempt to open listening socket'");
	...
	case CONNMAN_SERVER_STATUS_OPEN: {
	    if (AMX_DEBUG <= get_log_level()) debug("'connman_server_disconnect', '() ', 'closing listening socket...'");
...
data_event[connman_server_device] {
	...
	if (AMX_INFO <= get_log_level()) debug("'listening socket opened on port ', itoa(connman_server_config.port), ' ', IP_PROTOCOL_STRINGS[connman_server_config.protocol]");
    }
    offline: {
        connman_server_open_status = CONNMAN_SERVER_STATUS_CLOSED;
	
	if (AMX_INFO <= get_log_level()) debug("'listening socket on port ', itoa(connman_server_config.port), ' ', IP_PROTOCOL_STRINGS[connman_server_config.protocol], ' closed'");
	
	if (connman_server_auto_reopen) connman_server_open();
    }
    onerror: {
	if (AMX_ERROR <= get_log_level()) debug("'cculd not open listening socket on port ', itoa(connman_server_config.port), ' ', IP_PROTOCOL_STRINGS[connman_server_config.protocol], ' (Error ', itoa(data.number), ' ', ip_error_desc(type_cast(data.number)), ')'");
    ...
    string: {
	if (AMX_DEBUG <= get_log_level()) debug("'string received from ', data.sourceip, ' on port ', itoa(connman_server_config.port), ' ', IP_PROTOCOL_STRINGS[connman_server_config.protocol], ' (', itoa(length_string(data.text)), ' bytes): ', data.text");
    }
}
