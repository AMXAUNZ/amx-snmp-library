...
(*
    ...
    OID-<oid>,<value>,<type>,<community>,<agent address>,<request id>
    TRAP-<oid>,<value>,<type>,<community>,<agent address>
    ...
    send_command vdvSNMP, "'PROPERTY-SEND_DELAY,', itoa(<msec>)";						// Delay concurrent SNMP requests for slower devices. Default: 0ms
...
define_function long rawtol(char str[]) {
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'rawtol', '(', str, ')'");
    
    for (i = 1; i <= length_string(str); i++) {
	ret = (ret LSHIFT 8) + str[i];
    }

    // if (AMX_DEBUG <= get_log_level()) debug("'rawtol', '() ', 'returning ', ltoa(ret)");
    return ret;
}
...
define_function integer array_search(char needle[], char haystack[][]) {
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'array_search', '(', itoa(needle), ', ', 'haystack[', itoa(length_array(haystack)), ']', ')'");
    
    for (i = 1; i <= length_array(haystack); i++) {
	if (haystack[i] == "needle") {
	    // if (AMX_DEBUG <= get_log_level()) debug("'array_search', '() ', 'needle found. returning index ', itoa(i)");
	    return i;
	}
    }
    
    // if (AMX_DEBUG <= get_log_level()) debug("'array_search', '() ', 'needle could not be found. returning false'");
    return false;
}
...
define_function char[MAX_LEN_LONG_BASE128] base128_encode(long n) {
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'base128_encode', '(', itoa(n), ')'");
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'base128_encode', '() ', 'returning ', ret");
    return ret;
}
...
define_function long base128_decode(char str[MAX_LEN_LONG_BASE128]) {
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'base128_decode', '(', str, ')'");
    
    for (pos = 1; pos <= length_string(str); pos++) {
	ret = (ret LSHIFT 7) + str[pos];
    }
    
    // if (AMX_DEBUG <= get_log_level()) debug("'base128_decode', '() ', 'returning ', itoa(ret)");
    return ret;
}
...
define_function char[MAX_LEN_STR] implode(char parts[][], char delim[]) {
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'implode', '(', 'parts[', itoa(length_array(parts)), '], "', delim, '"', ')'");
    ...
    // if (AMX_DEBUG <= get_log_level()) debug("'implode', '() ', 'returning ', str");
    return str;
}
...
define_function integer setDebugState(integer lvl) {
	...
	send_string vdvModule, "'DEBUG-', lvl";
...
define_function reinitialize() {
	...
	if (AMX_INFO <= get_log_level()) debug("'reinitialize', '() ', 'default SNMP host and port not configured, and will need to be passed as command parameters.'");
    }
    if (!snmp_manager_trap_port) {
	if (AMX_INFO <= get_log_level()) debug("'reinitialize', '() ', 'SNMP trap port not configured. Trap listener disabled.'");
    }
...
define_function integer snmp_request(_connman_host host, _snmp_request request) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'snmp_request', '(', '_connman_host ', host.address, ':', itoa(host.port), ', ', request.pdu, ', ', request.oid, '@', request.community, ', ', request.tag.type, ', ', request.tag.contents, ')'");
	...
	if (AMX_ERROR <= get_log_level()) debug("'snmp_request', '() ', 'host, port, and community must be specified or configured!'");
	if (AMX_DEBUG <= get_log_level()) debug("'snmp_request', '() ', 'returning false'");
	return false;
    }
    
    if (length_string(connman_buffer_out[connman_buffer_out_pos_in].str)) {
	if (AMX_ERROR <= get_log_level()) debug("'snmp_request', '() ', 'outbound buffer is full. Cannot send message!'");
	if (AMX_DEBUG <= get_log_level()) debug("'snmp_request', '() ', 'returning false'");
	return false;
    }
    
    if (AMX_INFO <= get_log_level()) debug("'snmp_request', '() ', 'encoding message...'");
    ...
    if (!message) {
	if (AMX_ERROR <= get_log_level()) debug("'snmp_request', '() ', 'message could not be encoded!'");
	if (AMX_DEBUG <= get_log_level()) debug("'snmp_request', '() ', 'returning false'");
	return false;
    } else {
	if (AMX_INFO <= get_log_level()) debug("'snmp_request', '() ', 'message encoded in ', itoa(get_timer - timer), ' 1/10th sec'");
    }
    
    if (!connman_buffer_add_ex(host, message)) {
	if (AMX_ERROR <= get_log_level()) debug("'snmp_request', '() ', 'message could not be added to buffer!'");
    } else {
	if (AMX_INFO <= get_log_level()) debug("'snmp_request', '() ', 'message added to buffer'");
	connman_buffer_process();
    }
    
    if (AMX_DEBUG <= get_log_level()) debug("'snmp_request', '() ', 'returning true'");
    return true;
}
...
define_function char[MAX_LEN_OCTET_STRING] asn1_tag_encode(char type, char value[MAX_LEN_OCTET_STRING]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_encode', '(', type, ', ', value, ')'");
	...
	if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_encode', '() ', 'encoded tag of type ', format('$%02X', type), ' ', ASN1_TAG_STRINGS[array_search(type, ASN1_TAGS)], ' (', itoa(length_string(ret)), ' bytes)'");
    } else {
	if (AMX_WARNING <= get_log_level()) debug("'asn1_tag_encode', '() ', 'encoded tag of type ', format('$%02X', type), ' ', 'UNKNOWN', ' (', itoa(length_string(ret)), ' bytes)'");
    }

    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_encode', '() ', 'returning ', ret");
    return ret;
}
...
define_function char[MAX_LEN_ASN1_LENGTH_ENCODED] asn1_length_encode(long length) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_length_encode', '(', itoa(length), ')'");
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_length_encode', '() ', 'returning ', ret");
    return ret;
}
...
define_function char[MAX_LEN_OID_ENCODED] asn1_tag_oid_encode(char str[]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_oid_encode', '(', str, ')'");
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_oid_encode', '() ', 'returning ', ret");
    return ret;
}
...
define_function char[MAX_LEN_IPADDRESS_ENCODED] asn1_tag_ipaddress_encode(char str[]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_ipaddress_encode', '(', str, ')'");
    
    if (!explode(str, '.', false, octets)) {
	if (AMX_ERROR <= get_log_level()) debug("'asn1_tag_ipaddress_encode', '() ', 'could not parse address!', ret");
    } else {
	stack_var volatile integer i;
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_ipaddress_encode', '() ', 'returning ', ret");
    return ret;
}
...
define_function integer asn1_tag_decode(char str[], _tag tags[]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_decode', '(', str, ', ', 'tags[', itoa(max_length_array(tags)), ']', ')'");
	...
	if (array_search(tags[i].type, ASN1_TAGS)) {
	    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_decode', '() ', 'found tag ', itoa(i), ' of type ', format('$%02X', tags[i].type), ' ', ASN1_TAG_STRINGS[array_search(tags[i].type, ASN1_TAGS)], ': ', tags[i].contents, ' (', itoa(length), ' bytes)'");
	} else {
	    if (AMX_WARNING <= get_log_level()) debug("'asn1_tag_decode', '() ', 'found tag ', itoa(i), ' of type ', format('$%02X', tags[i].type), ' ', 'UNKNOWN', ': ', tags[i].contents, ' (', itoa(length), ' bytes)'");
	}
	...
	if (array_search(tags[i].type, ASN1_TAGS)) {
	    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_decode', '() ', 'decoded tag ', itoa(i), ' of type ', format('$%02X', tags[i].type), ' ', ASN1_TAG_STRINGS[array_search(tags[i].type, ASN1_TAGS)], ': ', tags[i].contents, ' (', itoa(length_string(tags[i].contents)), ' bytes)'");
	} else {
	    if (AMX_WARNING <= get_log_level()) debug("'asn1_tag_decode', '() ', 'decoded tag ', itoa(i), ' of type ', format('$%02X', tags[i].type), ' ', 'UNKNOWN', ': ', tags[i].contents, ' (', itoa(length_string(tags[i].contents)), ' bytes)'");
	}
    }
    
    set_length_array(tags, i);

    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_decode', '() ', 'returning ', itoa(i)");
    return i;
}
...
define_function char[MAX_LEN_OID] asn1_tag_oid_decode(char str[]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_oid_decode', '(', str, ')'");
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_oid_decode', '() ', 'returning ', ret");
    return ret;
}
...
define_function char[MAX_LEN_IPADDRESS] asn1_tag_ipaddress_decode(char str[]) {
    ...
    if (AMX_DEBUG <= get_log_level()) debug("'asn1_tag_ipaddress_decode', '(', str, ')'");
...
define_function connman_timeout_callback(_connman_host host, char str[]) {
    if (AMX_DEBUG <= get_log_level()) debug("'connman_timeout_callback', '(', '_connman_host ', host.address, ':', itoa(host.port), ', ', 'str[])'");
...
// connman_setProperty('HOST', '<address>');					// Server address.
// connman_setProperty('PORT', itoa(<port>));					// Server port.
// connman_setProperty('PROTOCOL', itoa(IP_UDP_2WAY));				// Transport protocol using NetLinx transport protocol constants.
connman_setProperty('CONNECT_DELAY', itoa(5000));				// Delay between connection attempts. Default: 5000ms.
connman_setProperty('SEND_DELAY', itoa(2000));					// Delay between concurrent strings sent from the outbound butter. Default: 0msec.
...
connman_setProperty('AUTO_DISCONNECT', booltostring(false));			// Automatically disconnect when all strings in the buffer have been sent. Default: false.
connman_setProperty('AUTO_RECONNECT', booltostring(false));			// Automatically re-connect if the connection drops. Also connects on reinitialise() without buffer contents. Default: false.
// connman_reinitialize();
...
data_event[vdvModule] {
	...
	switch (cmd) {
		...
		if (length_array(parts) < 1) {
		    if (AMX_ERROR <= get_log_level()) debug("'data_event command ', lower_string(cmd), ': ', 'could not parse parameters!'");
		    if (AMX_ERROR <= get_log_level()) debug("'data_event command ', lower_string(cmd), ': ', 'usage: ', cmd, '-<oid>[,<community>[,<host>[,<port>[,<request id>]]]]'");
		} else {
		...
		if (length_array(parts) < 3) {
		    if (AMX_ERROR <= get_log_level()) debug("'data_event command ', lower_string(cmd), ': ', 'could not parse parameters!'");
		    if (AMX_ERROR <= get_log_level()) debug("'data_event command ', lower_string(cmd), ': ', 'usage: SNMPSET-<oid>,<type>,<value>[,<community>[,<host>[,<port>[,<request id>]]]]'");
		} else {
		    stack_var volatile _snmp_request request;
		    stack_var volatile _connman_host host;
	    ...
	    default: {
		if (AMX_ERROR <= get_log_level()) debug("'data_event command: ', 'unknown command: ', cmd");
	    }
	}
    }
}
...
data_event[connman_server_device] {
	...
	if (AMX_INFO <= get_log_level()) debug("'response: ', 'message processing...'");
	timer = get_timer;
	
	asn1_tag_decode(data.text, message);
	asn1_tag_decode(message[SNMP_FIELD_MESSAGE].contents, snmp_message)
	
	if (AMX_INFO <= get_log_level()) debug("'response: ', 'version: ',   snmp_message[SNMP_MESSAGE_FIELD_VERSION].contents");
	if (AMX_INFO <= get_log_level()) debug("'response: ', 'community: ', snmp_message[SNMP_MESSAGE_FIELD_COMMUNITY].contents");
	if (AMX_INFO <= get_log_level()) debug("'response: ', 'pdu: ',       ASN1_TAG_STRINGS[array_search(snmp_message[SNMP_MESSAGE_FIELD_PDU].type, ASN1_TAGS)]");
	...
	switch (snmp_message[SNMP_MESSAGE_FIELD_PDU].type) {
		...
		if (AMX_DEBUG <= get_log_level()) debug("'response: ', 'processing response PDU...'");
		
		asn1_tag_decode(snmp_message[SNMP_MESSAGE_FIELD_PDU].contents, snmp_pdu);
		
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'request id: ', snmp_pdu[SNMP_PDU_FIELD_REQUEST_ID].contents");
		...
		if (snmp_pdu[SNMP_PDU_FIELD_ERROR].contents != itoa(SNMP_ERROR_NO_ERROR)) {
			...
			if (AMX_ERROR <= get_log_level()) debug("'response: ', 'error index: ', snmp_pdu[SNMP_PDU_FIELD_ERROR_INDEX].contents, ' (', snmp_varbind[SNMP_VARBIND_FIELD_OID].contents, ')'");
			...
			if (AMX_ERROR <= get_log_level()) debug("'response: ', 'error: ', snmp_pdu[SNMP_PDU_FIELD_ERROR].contents, ' ', SNMP_ERROR_STRINGS[atoi(snmp_pdu[SNMP_PDU_FIELD_ERROR].contents) + 1]");
		...
		for (i = 1; i <= length_array(snmp_varbind_list); i++) {
		    ...
		    if (AMX_INFO <= get_log_level()) debug("'response: ', 'OID ', snmp_varbind[SNMP_VARBIND_FIELD_OID].contents, ' = ', snmp_varbind[SNMP_VARBIND_FIELD_VALUE].contents, ' (', ASN1_TAG_STRINGS[array_search(snmp_varbind[SNMP_VARBIND_FIELD_VALUE].type, ASN1_TAGS)], ')'");
		    
		    send_string vdvModule, "'OID', '-', snmp_varbind[SNMP_VARBIND_FIELD_OID].contents, ',', snmp_varbind[SNMP_VARBIND_FIELD_VALUE].contents, ',', ASN1_TAG_STRINGS[array_search(snmp_varbind[SNMP_VARBIND_FIELD_VALUE].type, ASN1_TAGS)], ',', snmp_message[SNMP_MESSAGE_FIELD_COMMUNITY].contents, ',', data.sourceip, ',', snmp_pdu[SNMP_PDU_FIELD_REQUEST_ID].contents";
		}
	    }
		...
		if (AMX_DEBUG <= get_log_level()) debug("'response: ', 'processing trap PDU...'");
		
		asn1_tag_decode(snmp_message[SNMP_MESSAGE_FIELD_PDU].contents, snmp_pdu);
		
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'enterprise: ',    snmp_pdu[SNMP_PDU_FIELD_ENTERPRISE].contents");
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'agent address: ', snmp_pdu[SNMP_PDU_FIELD_AGENT_ADDR].contents");
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'generic trap: ',  snmp_pdu[SNMP_PDU_FIELD_GENERIC_TRAP].contents");
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'specific trap: ', snmp_pdu[SNMP_PDU_FIELD_SPECIFIC_TRAP].contents");
		if (AMX_INFO <= get_log_level()) debug("'response: ', 'time stamp: ',    snmp_pdu[SNMP_PDU_FIELD_TIME_STAMP].contents");
		...
		for (i = 1; i <= length_array(snmp_varbind_list); i++) {
		    ...
		    send_string vdvModule, "'TRAP', '-', snmp_varbind[SNMP_VARBIND_FIELD_OID].contents, ',', snmp_varbind[SNMP_VARBIND_FIELD_VALUE].contents, ',', ASN1_TAG_STRINGS[array_search(snmp_varbind[SNMP_VARBIND_FIELD_VALUE].type, ASN1_TAGS)], ',', snmp_message[SNMP_MESSAGE_FIELD_COMMUNITY].contents, ',', snmp_pdu[SNMP_PDU_FIELD_AGENT_ADDR].contents";
		}
	    }
	}
	
	if (AMX_INFO <= get_log_level()) debug("'response: ', 'message processed in ', itoa(get_timer - timer), ' 1/10th sec'");
    }
}
